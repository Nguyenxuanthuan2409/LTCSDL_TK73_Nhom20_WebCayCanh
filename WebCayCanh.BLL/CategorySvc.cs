...
namespace WebCayCanh.BLL
{
    ...
    public class CategorySvc : GenericSvc<CategoryRep, Category>
    {
        ...
        public override SingleRsp Update(Category m)
        {
            ...
            try
            {
                var m1 = m.CategoryId != null ? _rep.Read(m.CategoryId) : _rep.Read(m.CategoryDescription);
                if (m1 == null)
                {
       ...
       public SingleRsp CreateCategory(CategoryReq req)
        {
            var res = new SingleRsp();
            try
            {
        ...
        public object SearchCategory(string keyWord, int size, int page)
        {
            ...
            if (!string.IsNullOrEmpty(keyWord)) //Kiểm tra keyWord có trống không
            {
                ...
                query = All.Where(x => x.CategoryId.Contains(keyWord) || x.CategoryName.Contains(keyWord));
            }
        ...
        public SingleRsp DeleteCategory(String id)
        {
            var res = new SingleRsp();
            try
            {
