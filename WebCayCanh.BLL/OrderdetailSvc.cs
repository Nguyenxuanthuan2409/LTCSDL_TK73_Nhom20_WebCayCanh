...
namespace WebCayCanh.BLL
{
    ...
    public class OrderdetailSvc: GenericSvc<OrderdetailRep, Orderdetail>
    {
        ...
        public override SingleRsp Update(Orderdetail m)
        {
            ...
            try
            {
                var m1 = m.OrderdetailId != null ? _rep.Read(m.OrderdetailId) : _rep.Read(m.OrderdetailDescription);
                if (m1 == null)
                {
        ...
        public SingleRsp CreateOrdertail(OrderdetailReq req)
        {
            var res = new SingleRsp();
            try
            {
                Orderdetail c = new Orderdetail();
                c.OrderdetailId = req.OrderdetailId;
                c.Price = req.Price;
                c.Quantity = req.Quantity;
                c.OrderId = req.OrderId;
                c.ProductId = req.ProductId;
                c.Userid = req.Userid;
                c.OrderdetailDescription = req.OrderdetailDescription;
        ...
        public SingleRsp UpdateOrderdetail(OrderdetailReq req)
        {
            var res = new SingleRsp();
            try
            {
        ...
        public object SearchOrderdetail(string keyWord, int size, int page)
        {
            ...
            if (!string.IsNullOrEmpty(keyWord)) //Kiểm tra keyWord có trống không
            {
                ...
                query = All.Where(x => x.OrderdetailId.Contains(keyWord));
            }
        ...
        public SingleRsp DeleteOrderdetail(String id)
        {
            var res = new SingleRsp();
            try
            {
