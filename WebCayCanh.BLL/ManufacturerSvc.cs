...
namespace WebCayCanh.BLL
{
    ...
    public class ManufacturerSvc : GenericSvc<ManufacturerRep, Manufacturer>
    {
        #region -- Overrides --
        ...
        public override SingleRsp Update(Manufacturer m)
        {
            var res = new SingleRsp();
            try
            {
                var m1 = m.ManufacturerId != null ? _rep.Read(m.ManufacturerId) : _rep.Read(m.ManufacturerDescription);
                if (m1 == null)
                {
        ...
        public SingleRsp CreateManufacturer(ManufacturerReq req)
        {
            var res = new SingleRsp();
            try
            {
                ...
                c.ManufacturerName = req.ManufacturerName;
                c.ManufacturerDescription = req.ManufacturerDescription;
        ...
        public SingleRsp UpdateManufacturer(ManufacturerReq req)
        {
            var res = new SingleRsp();
            try
            {
        ...
        public object SearchManufacturer(string keyWord, int size, int page)
        {
            ...
            if (!string.IsNullOrEmpty(keyWord)) //Kiểm tra keyWord có trống không
            {
                ...
                query = All.Where(x => x.ManufacturerId.Contains(keyWord) || x.ManufacturerName.Contains(keyWord));
            }
        ...
        public SingleRsp DeleteManufacturer(String id)
        {
            var res = new SingleRsp();
            try
            {
